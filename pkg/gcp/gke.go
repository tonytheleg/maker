...
// CreateGkeClusterRole does stuff
func CreateGkeClusterRole(sess *session.Session) (string, error) {
...
func CreateGkeCluster(sess *session.Session, name, arn string, subnets []string) error {
	digest := md5.New()
...
func CreateGkeNodeGroup(sess *session.Session, name, arn, nodeSize string, nodeCount int, subnets []string) error {
	// pre-check
...
func PrintGkeClusterStatus(sess *session.Session, clusterName, nodeGroupName string) error {
	svc := eks.New(sess)
	input := &eks.DescribeClusterInput{
...
func DeleteGkeNodeGroup(sess *session.Session, clusterName, nodeGroupName string) error {
	svc := eks.New(sess)
...
func DeleteGkeCluster(sess *session.Session, name, nodeGroupName string) error {
	// pre-check
	for {
